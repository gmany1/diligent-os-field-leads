            ...
            {[
              ...
              <div key={i} className="p-4 space-y-2">
                <div className="flex justify-between items-center">
                  <span className={"px-2 py-0.5 rounded text-xs font-bold " + (log.level === 'INFO' ? 'bg-blue-100 text-blue-800' : log.level === 'WARN' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800')}>
                    {log.level}
                  </span>
                  <span className="text-xs font-mono text-gray-500">{log.time.split(' ')[1]}</span>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase">{log.service}</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{log.msg}</p>
                </div>
              </div>
            ))}
          </div>
